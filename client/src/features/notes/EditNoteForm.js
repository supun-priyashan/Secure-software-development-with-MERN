...
const EditNoteForm = ({ note, users }) => {
    ...
    const [title, setTitle] = useState(decryptMessage(note.title))
    const [text, setText] = useState(decryptMessage(note.text))
    ...
    const onSaveNoteClicked = async (e) => {
        if (canSave) {
            ...
            await updateNote({ id: note.id, title: ttl, text: txt })
        }
    }
        ...
        <main className="loginn" style={{textAlign: 'center', paddingTop: '50px', paddingLeft: '300px', paddingRight: '100px'}}>
            ...
            <form className="form" onSubmit={e => e.preventDefault()}>
                <div className="form__title-row">
                    <h2>Edit Message #{note.ticket}</h2>
